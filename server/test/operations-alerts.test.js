...
test('operations alerts expose real backlog, failures, and delayed work without request content', async (t) => {
  ...
  await db.mutate((data) => {
    ...
    for (let index = 0; index < 7; index += 1) data.generationJobs.push({ id: `failed-${index}`, userId: admin.id, apiId: 'api-safe', modelId: 'video', status: 'failed', prompt: `secret-failure-${index}`, createdAt: new Date(now - 10_000).toISOString(), updatedAt: new Date(now - 5_000).toISOString() });
  ...
  assert.equal(metrics.recent.failureRate, Number((7 / 33).toFixed(4)));
