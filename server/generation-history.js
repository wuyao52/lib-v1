...
const retentionMs = (type = 'video') => {
  if (type === 'image' || type === 'img2img') return IMAGE_RETENTION_DAYS * 24 * 60 * 60 * 1000;
...
export async function cleanupExpiredGenerationHistory({ db, now = new Date() }) {
  const nowMs = now.getTime();
  let removed = 0;
  let shortened = 0;
  const needsCleanup = db.read('generationHistory').some((item) => {
    const imageExpiry = Date.parse(item.createdAt) + retentionMs(item.type);
    return Date.parse(item.expiresAt) <= nowMs
      || ((item.type === 'image' || item.type === 'img2img') && Date.parse(item.expiresAt) > imageExpiry);
  });
  if (!needsCleanup) return { removed, shortened };
  await db.mutateCollections(['generationHistory'], (data) => {
    data.generationHistory.forEach((item) => {
      if (item.type !== 'image' && item.type !== 'img2img') return;
      const maximumExpiry = Date.parse(item.createdAt) + retentionMs(item.type);
      if (Date.parse(item.expiresAt) > maximumExpiry) {
        item.expiresAt = new Date(maximumExpiry).toISOString();
        shortened += 1;
      }
    });
    const before = data.generationHistory.length;
    data.generationHistory = data.generationHistory.filter((item) => Date.parse(item.expiresAt) > nowMs);
    removed = before - data.generationHistory.length;
  });
...
export function registerGenerationHistoryRoutes(router, { db, requireAuth }) {
  ...
  router.get('/', async (req, res) => {
    const now = Date.now();
    await cleanupExpiredGenerationHistory({ db, now: new Date(now) });
    ...
    const needsRepair = db.read('generationJobs').some((job) => job.status === 'completed' && job.resultUrl && !knownHistory.has(`${job.userId}:${job.resultUrl}`));
    if (needsRepair) await db.mutateCollections(['generationHistory'], (data) => {
      ...
      data.generationJobs.filter((job) => job.status === 'completed' && job.resultUrl && !known.has(`${job.userId}:${job.resultUrl}`)).forEach((job) => {
        ...
        data.generationHistory.push({ id: randomUUID(), userId: job.userId, projectId: job.projectId || '', nodeId: job.nodeId || null, type: 'video', prompt: job.prompt || '', url: job.resultUrl, thumbnail: job.thumbnail || null, createdAt, expiresAt: new Date(Date.parse(createdAt) + retentionMs('video')).toISOString() });
      });
  ...
  router.post('/', async (req, res) => {
    await cleanupExpiredGenerationHistory({ db });
    ...
    const type = String(req.body.type || 'video').slice(0, 20);
    const record = { id: randomUUID(), userId: req.user.id, projectId: String(req.body.projectId || '').slice(0, 100), nodeId: String(req.body.nodeId || '').slice(0, 100) || null, type, prompt: String(req.body.prompt || '').slice(0, 10000), url, thumbnail: String(req.body.thumbnail || '').trim() || null, createdAt: createdAt.toISOString(), expiresAt: new Date(createdAt.getTime() + retentionMs(type)).toISOString() };
