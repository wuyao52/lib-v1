...
const TABLES = {
  ...
  modelPricing: {
    ...
    create: `CREATE TABLE IF NOT EXISTS model_pricing (
      ...
      allowed_qualities JSON NULL,
      default_quality VARCHAR(32) NULL,
    ...
    select: 'SELECT id, api_id AS apiId, model_id AS modelId, display_name AS displayName, category, billing_unit AS billingUnit, unit_price_cents AS unitPriceCents, min_duration_sec AS minDurationSec, max_duration_sec AS maxDurationSec, allowed_durations_sec AS allowedDurationsSec, allowed_resolutions AS allowedResolutions, allowed_qualities AS allowedQualities, default_quality AS defaultQuality, quality_required AS qualityRequired, max_reference_images AS maxReferenceImages, max_reference_audios AS maxReferenceAudios, max_reference_videos AS maxReferenceVideos, enabled, created_at AS createdAt, updated_at AS updatedAt FROM model_pricing',
    insert: 'INSERT INTO model_pricing (id, api_id, model_id, display_name, category, billing_unit, unit_price_cents, min_duration_sec, max_duration_sec, allowed_durations_sec, allowed_resolutions, allowed_qualities, default_quality, quality_required, max_reference_images, max_reference_audios, max_reference_videos, enabled, created_at, updated_at) VALUES ?',
    values: (row) => [row.id, row.apiId, row.modelId, row.displayName, row.category, row.billingUnit, row.unitPriceCents, row.minDurationSec || null, row.maxDurationSec || null, JSON.stringify(row.allowedDurationsSec || []), JSON.stringify(row.allowedResolutions || []), JSON.stringify(row.allowedQualities || []), row.defaultQuality || null, row.qualityRequired ? 1 : 0, Number.isInteger(Number(row.maxReferenceImages)) ? Number(row.maxReferenceImages) : 4, Number.isInteger(Number(row.maxReferenceAudios)) ? Number(row.maxReferenceAudios) : 0, Number.isInteger(Number(row.maxReferenceVideos)) ? Number(row.maxReferenceVideos) : 0, row.enabled ? 1 : 0, row.createdAt, row.updatedAt],
    parse: (row) => ({ ...row, enabled: Boolean(row.enabled), qualityRequired: Boolean(row.qualityRequired), allowedDurationsSec: typeof row.allowedDurationsSec === 'string' ? JSON.parse(row.allowedDurationsSec || '[]') : (row.allowedDurationsSec || []), allowedResolutions: typeof row.allowedResolutions === 'string' ? JSON.parse(row.allowedResolutions || '[]') : (row.allowedResolutions || []), allowedQualities: typeof row.allowedQualities === 'string' ? JSON.parse(row.allowedQualities || '[]') : (row.allowedQualities || []) }),
  },
  userModelAccess: {
