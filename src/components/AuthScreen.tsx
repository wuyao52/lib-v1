        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {mode === 'register' ? (
              ...
              <label className="block"><span className="text-sm text-dark-300">图片验证码</span><div className="mt-2 grid grid-cols-[1fr_150px] gap-2"><input value={captchaCode} onChange={(e) => setCaptchaCode(e.target.value.replace(/\D/g, '').slice(0, 5))} inputMode="numeric" autoComplete="off" pattern="\d{5}" maxLength={5} required placeholder="输入图中数字" className="w-full h-11 px-3 rounded-lg bg-dark-900 border border-dark-700 focus:border-primary-500 outline-none tracking-[0.25em]" /><button type="button" onClick={loadCaptcha} disabled={isLoadingCaptcha} title="刷新验证码" aria-label="刷新验证码" className="h-11 w-[150px] overflow-hidden rounded-lg border border-dark-600 bg-dark-800 hover:border-dark-500 disabled:opacity-50 flex items-center justify-center">{isLoadingCaptcha ? <Loader2 className="w-5 h-5 animate-spin" /> : captchaImage ? <img src={captchaImage} alt="数字验证码" className="w-full h-full object-cover" /> : <RefreshCw className="w-5 h-5" />}</button></div></label>
            )}
