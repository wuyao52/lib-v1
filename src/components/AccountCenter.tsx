...
type Pricing = { id: string; apiId: string; modelId: string; displayName: string; category: string; billingUnit: string; unitPriceCents: number; minDurationSec?: number | null; maxDurationSec?: number | null; allowedDurationsSec?: number[]; allowedResolutions?: string[]; maxReferenceImages?: number; enabled: boolean };
...
const emptyPrice = { apiId: '', modelId: '', displayName: '', category: 'text', billingUnit: 'request', priceYuan: '', minDurationSec: '', maxDurationSec: '', allowedDurations: '', allowedResolutions: '', maxReferenceImages: '4' };
      ...
      <div className="overflow-y-auto p-5 space-y-6">
        ...
        {mode === 'billing' ? <BillingView balance={balance} transactions={transactions} recharges={recharges} amount={amount} setAmount={setAmount} providers={paymentProviders} provider={paymentProvider} setProvider={setPaymentProvider} orders={paymentOrders} startPayment={startPayment} /> : <>
          ...
          <section>
            ...
            <div className="mt-3 divide-y divide-dark-700">{pricing.map((item) => <div key={item.id} className="py-2 flex justify-between gap-3 text-sm"><span className="text-dark-200 min-w-0">{item.displayName} <small className="text-dark-500">{item.modelId} · {item.category}{item.category === 'video' && (item.allowedDurationsSec?.length ? ` · 仅 ${item.allowedDurationsSec.join('、')} 秒` : (item.minDurationSec || item.maxDurationSec) ? ` · ${item.minDurationSec || 1}-${item.maxDurationSec || 3600} 秒` : '')}{item.category === 'video' && item.allowedResolutions?.length ? ` · ${item.allowedResolutions.join('、')}` : ''}{item.category === 'video' ? ` · 最多 ${Number.isInteger(item.maxReferenceImages) ? item.maxReferenceImages : 4} 张参考图` : ''}</small></span><span className="flex items-center gap-3 text-green-400 shrink-0">{yuan(item.unitPriceCents)} / {item.billingUnit === 'second' ? '秒' : item.billingUnit === 'image' ? '张' : '次'}<button title={item.enabled ? '取消发布' : '发布'} className={item.enabled ? 'text-green-400' : 'text-dark-500'} onClick={() => void togglePricingPublished(item)}>{item.enabled ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}</button><span className={`text-[11px] ${item.enabled ? 'text-green-400' : 'text-dark-500'}`}>{item.enabled ? '已发布' : '未发布'}</span><button title="编辑" className="text-primary-400" onClick={() => { setEditingPriceId(item.id); setPriceForm({ apiId: item.apiId, modelId: item.modelId, displayName: item.displayName, category: item.category, billingUnit: item.billingUnit, priceYuan: String(item.unitPriceCents / 100), minDurationSec: String(item.minDurationSec || ''), maxDurationSec: String(item.maxDurationSec || ''), allowedDurations: (item.allowedDurationsSec || []).join(','), allowedResolutions: (item.allowedResolutions || []).join(','), maxReferenceImages: String(Number.isInteger(item.maxReferenceImages) ? item.maxReferenceImages : 4) }); void loadModelsForPricing(item.apiId, false); }}><Pencil className="w-4 h-4" /></button><button title="删除" className="text-red-400" onClick={() => act(() => apiRequest(`/api/admin/pricing/${item.id}`, { method: 'DELETE' }), '定价已删除')}><Trash2 className="w-4 h-4" /></button></span></div>)}</div>
          </section>
