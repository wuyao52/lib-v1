...
function AdminUsers({ users, currentUserId, recharges, pricing, accessUserId, accessDraft, setAccessDraft, balanceAdjustments, setBalanceAdjustments, currentPassword, act, loadModelAccess, saveModelAccess }: AdminUsersProps) {
  ...
  return <section className="grid lg:grid-cols-2 gap-6">
    <div><h3 className="text-sm font-medium text-white mb-2">用户、权限与余额</h3><div className="divide-y divide-dark-700">{users.map((item: AuthUser) => <div key={item.id} className={`py-2 grid grid-cols-[1fr_auto] gap-2 items-center text-sm ${item.id === accessUserId ? 'bg-primary-500/10 border-l-2 border-primary-400 pl-2' : ''}`}><span className="text-dark-200">{item.username}{item.id === accessUserId && <span className="ml-2 text-[10px] text-primary-300">当前配置中</span>}<small className="block text-dark-500">{item.email} · {yuan(item.balanceCents)} · {item.accountType === 'special' ? '特殊用户' : item.role === 'system' ? '系统用户' : '普通用户'}</small></span><div className="flex gap-2"><button title="配置特殊用户模型" className={`px-2 py-1 border ${item.id === accessUserId ? 'border-primary-300 bg-primary-500/20 text-white' : 'border-primary-500 text-primary-300'}`} onClick={() => void loadModelAccess(item.id)}>{item.id === accessUserId ? '正在配置' : '模型权限'}</button><div className="flex"><input className="w-24 px-2 py-1 bg-dark-900 border border-dark-600 rounded-l text-white" type="number" step="0.01" value={balanceAdjustments[item.id] || ''} onChange={(e) => setBalanceAdjustments({ ...balanceAdjustments, [item.id]: e.target.value })} placeholder="增减元" /><button title="调整余额" className="px-2 bg-dark-700 border border-dark-600 rounded-r text-green-400" onClick={() => act(() => apiRequest(`/api/admin/users/${item.id}/balance`, { method: 'POST', body: JSON.stringify({ amountCents: Math.round(Number(balanceAdjustments[item.id]) * 100), description: '系统后台调整余额', currentPassword }) }), '用户余额已调整')}><Coins className="w-4 h-4" /></button></div><select className="bg-dark-900 border border-dark-600 rounded px-2 py-1 text-dark-200" disabled={item.id === currentUserId} value={item.role} onChange={(e) => act(() => apiRequest(`/api/admin/users/${item.id}/role`, { method: 'PATCH', body: JSON.stringify({ role: e.target.value, currentPassword }) }), '用户角色已更新')}><option value="user">普通用户</option><option value="system">系统用户</option></select></div></div>)}</div>
      {accessUserId && <div className="mt-4 border border-primary-500/40 p-3 space-y-2"><div className="flex items-center justify-between gap-2"><div><div className="text-sm text-white">特殊用户模型权限</div><div className="text-xs text-primary-300">当前用户：{selectedUser?.username || '未知用户'} · {selectedUser?.email || '未知邮箱'}</div></div><span className="text-[10px] text-green-300">已读取配置</span></div>{pricing.map((item) => { const draft = accessDraft[item.id] || { enabled: false, price: String(item.unitPriceCents / 100) }; return <label key={item.id} className={`flex items-center gap-2 text-xs ${draft.enabled ? 'text-white' : 'text-dark-500'}`}><input type="checkbox" checked={draft.enabled} onChange={(event) => setAccessDraft((current) => ({ ...current, [item.id]: { ...draft, enabled: event.target.checked } }))} /><span className="flex-1">{item.displayName}</span><span className={draft.enabled ? 'text-green-300' : 'text-dark-600'}>{draft.enabled ? '已选择' : '未选择'}</span><input aria-label={`${item.displayName}专属价格`} className="w-20 px-1 py-1 bg-dark-900 border border-dark-600 text-white" type="number" min="0" step="0.01" value={draft.price} onChange={(event) => setAccessDraft((current) => ({ ...current, [item.id]: { ...draft, price: event.target.value } }))} /><span className="text-dark-500">元</span></label>; })}<div className="flex items-center justify-between pt-1"><span className="text-[10px] text-amber-300">勾选和价格修改后，请点击保存才会生效</span><button onClick={() => void saveModelAccess()} className="px-3 py-1.5 bg-primary-600 text-white text-xs">保存模型权限</button></div></div>}
    </div><div><h3 className="text-sm font-medium text-white mb-2">充值审核</h3><div className="divide-y divide-dark-700">{recharges.filter((item: Recharge) => item.status === 'pending').map((item: Recharge) => <div key={item.id} className="py-2 flex justify-between items-center text-sm"><span className="text-dark-200">{item.user?.username || item.user?.email}<small className="block text-dark-500">{yuan(item.amountCents)} · {item.note || '无备注'}</small></span><div className="flex gap-1"><button title="通过并入账" className="p-2 text-green-400" onClick={() => act(() => apiRequest(`/api/admin/recharges/${item.id}/review`, { method: 'POST', body: JSON.stringify({ decision: 'approved' }) }), '充值已入账')}><Check className="w-4 h-4" /></button><button title="拒绝" className="p-2 text-red-400" onClick={() => act(() => apiRequest(`/api/admin/recharges/${item.id}/review`, { method: 'POST', body: JSON.stringify({ decision: 'rejected' }) }), '充值申请已拒绝')}><X className="w-4 h-4" /></button></div></div>)}</div></div></section>;
}
