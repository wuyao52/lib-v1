        ...
        {mode === 'billing' ? <BillingView balance={balance} transactions={transactions} recharges={recharges} amount={amount} setAmount={setAmount} providers={paymentProviders} provider={paymentProvider} setProvider={setPaymentProvider} orders={paymentOrders} startPayment={startPayment} /> : <>
          ...
          <section>
            ...
            <div className="grid lg:grid-cols-[240px_1fr] gap-4 border border-dark-600 rounded p-4 bg-dark-900/30">
              ...
              {priceForm.category === 'image' && <div className="space-y-1 lg:col-start-2"><span className="text-xs text-dark-300">图片模型分辨率</span><input aria-label="图片模型分辨率" className={field} value={priceForm.allowedResolutions} onChange={(e) => setPriceForm({ ...priceForm, allowedResolutions: e.target.value })} placeholder="如 1080p；多个分辨率用逗号分隔" /><p className="text-[10px] text-dark-500">自动识别会读取供应商模型目录；一个值表示固定分辨率，多个值表示用户可选。</p></div>}
             </div>
