...
type Pricing = { id: string; apiId: string; modelId: string; displayName: string; category: string; billingUnit: string; unitPriceCents: number; minDurationSec?: number | null; maxDurationSec?: number | null; allowedDurationsSec?: number[]; allowedResolutions?: string[]; enabled: boolean };
...
const emptyPrice = { apiId: '', modelId: '', displayName: '', category: 'text', billingUnit: 'request', priceYuan: '', minDurationSec: '', maxDurationSec: '', allowedDurations: '', allowedResolutions: '' };
  ...
  const submitPricing = async () => {
    ...
    try {
      await apiRequest(editingPriceId ? `/api/admin/pricing/${editingPriceId}` : '/api/admin/pricing', {
        ...
        body: JSON.stringify({ ...priceForm, unitPriceCents, minDurationSec: fixedDurations.length ? null : priceForm.minDurationSec, maxDurationSec: fixedDurations.length ? null : priceForm.maxDurationSec, allowedDurationsSec: fixedDurations, allowedResolutions: priceForm.allowedResolutions.split(',').map((value) => value.trim()).filter(Boolean) }),
      });
