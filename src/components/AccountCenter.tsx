...
type Pricing = { id: string; apiId: string; modelId: string; displayName: string; category: string; billingUnit: string; unitPriceCents: number; minDurationSec?: number | null; maxDurationSec?: number | null; allowedDurationsSec?: number[]; allowedResolutions?: string[]; allowedQualities?: string[]; defaultQuality?: string; qualityRequired?: boolean; maxReferenceImages?: number; maxReferenceAudios?: number; maxReferenceVideos?: number; enabled: boolean };
type DiscoveredModel = { id: string; name: string; type: string; supportedResolutions?: string[]; allowedDurationsSec?: number[]; supportedRatios?: string[]; allowedQualities?: string[]; defaultQuality?: string; qualityRequired?: boolean; maxReferenceImages?: number; maxReferenceAudios?: number; maxReferenceVideos?: number };
type QueueOverview = {
...
const emptyPrice = { apiId: '', modelId: '', displayName: '', category: 'text', billingUnit: 'request', priceYuan: '', minDurationSec: '', maxDurationSec: '', allowedDurations: '', allowedResolutions: '', allowedQualities: '', defaultQuality: '', qualityRequired: false, maxReferenceImages: '4', maxReferenceAudios: '0', maxReferenceVideos: '0' };
...
function AccountCenterContent({ mode, onClose }: { mode: 'billing' | 'admin'; onClose: () => void }) {
  ...
  const submitPricing = async () => {
    ...
    try {
      await apiRequest(editingPriceId ? `/api/admin/pricing/${editingPriceId}` : '/api/admin/pricing', {
        ...
        body: JSON.stringify({ ...priceForm, unitPriceCents, minDurationSec: fixedDurations.length ? null : priceForm.minDurationSec, maxDurationSec: fixedDurations.length ? null : priceForm.maxDurationSec, allowedDurationsSec: fixedDurations, allowedResolutions: priceForm.allowedResolutions.split(',').map((value) => value.trim()).filter(Boolean), allowedQualities: priceForm.allowedQualities.split(',').map((value) => value.trim()).filter(Boolean), maxReferenceImages: priceForm.maxReferenceImages, maxReferenceAudios: priceForm.maxReferenceAudios, maxReferenceVideos: priceForm.maxReferenceVideos }),
      });
  ...
  const pricingInApiOrder: Pricing[] = [];
        ...
        {mode === 'billing' ? <BillingView balance={balance} transactions={transactions} recharges={recharges} amount={amount} setAmount={setAmount} providers={paymentProviders} provider={paymentProvider} setProvider={setPaymentProvider} orders={paymentOrders} startPayment={startPayment} /> : <>
          ...
          <section>
            ...
            <div className="grid lg:grid-cols-[240px_1fr] gap-4 border border-dark-600 rounded p-4 bg-dark-900/30">
              ...
              {priceForm.category === 'image' && <div className="space-y-2 lg:col-start-2"><div className="space-y-1"><span className="text-xs text-dark-300">图片模型分辨率</span><input aria-label="图片模型分辨率" className={field} value={priceForm.allowedResolutions} onChange={(e) => setPriceForm({ ...priceForm, allowedResolutions: e.target.value })} placeholder="如 1080p；多个分辨率用逗号分隔" /><p className="text-[10px] text-dark-500">一个值表示固定分辨率，多个值表示用户可选。</p></div><div className="space-y-1"><span className="text-xs text-dark-300">图片模型质量</span><input aria-label="图片模型质量" className={field} value={priceForm.allowedQualities} onChange={(e) => setPriceForm({ ...priceForm, allowedQualities: e.target.value })} placeholder="如 standard,high；留空表示不发送 quality" /><p className="text-[10px] text-dark-500">只填写端口明确支持的值，不要猜测 auto。</p></div><div className="grid grid-cols-[1fr_auto] items-end gap-2"><label className="space-y-1"><span className="text-xs text-dark-300">默认质量</span><input aria-label="图片模型默认质量" className={field} value={priceForm.defaultQuality} onChange={(e) => setPriceForm({ ...priceForm, defaultQuality: e.target.value })} placeholder="可空" /></label><label className="flex items-center gap-2 pb-2 text-xs text-dark-300"><input type="checkbox" checked={priceForm.qualityRequired} onChange={(e) => setPriceForm({ ...priceForm, qualityRequired: e.target.checked })} />质量必填</label></div></div>}
             </div>
            <div className="mt-3 divide-y divide-dark-700">{pricingInApiOrder.map((item) => <div key={item.id} className="py-2 flex justify-between gap-3 text-sm"><span className="text-dark-200 min-w-0">{item.displayName} <small className="text-dark-500">{item.modelId} · {item.category}</small></span><span className="flex items-center gap-2 text-green-400 shrink-0"><input aria-label={`${item.displayName}价格`} className="w-20 px-2 py-1 bg-dark-900 border border-dark-600 text-white" type="number" min="0" step="0.01" value={priceDrafts[item.id] ?? ''} onChange={(event) => setPriceDrafts((current) => ({ ...current, [item.id]: event.target.value }))} /> / {item.billingUnit === 'second' ? '秒' : item.billingUnit === 'image' ? '张' : '次'}<button title={item.enabled ? '取消发布' : '发布'} className={item.enabled ? 'text-green-400' : 'text-dark-500'} onClick={() => void togglePricingPublished(item)}>{item.enabled ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}</button><button title="编辑" className="text-primary-400" onClick={() => { setEditingPriceId(item.id); setPriceForm({ apiId: item.apiId, modelId: item.modelId, displayName: item.displayName, category: item.category, billingUnit: item.billingUnit, priceYuan: String(item.unitPriceCents / 100), minDurationSec: String(item.minDurationSec || ''), maxDurationSec: String(item.maxDurationSec || ''), allowedDurations: (item.allowedDurationsSec || []).join(','), allowedResolutions: (item.allowedResolutions || []).join(','), allowedQualities: (item.allowedQualities || []).join(','), defaultQuality: item.defaultQuality || '', qualityRequired: Boolean(item.qualityRequired), maxReferenceImages: String(Number.isInteger(item.maxReferenceImages) ? item.maxReferenceImages : 4), maxReferenceAudios: String(Number.isInteger(item.maxReferenceAudios) ? item.maxReferenceAudios : 0), maxReferenceVideos: String(Number.isInteger(item.maxReferenceVideos) ? item.maxReferenceVideos : 0) }); void loadModelsForPricing(item.apiId, false); }}><Pencil className="w-4 h-4" /></button><button title="删除" className="text-red-400" onClick={() => act(() => apiRequest(`/api/admin/pricing/${item.id}`, { method: 'DELETE' }), '定价已删除')}><Trash2 className="w-4 h-4" /></button></span></div>)}</div><button onClick={() => void savePricingBatch()} className="mt-3 px-3 py-2 bg-primary-600 text-white text-sm">统一更新价格</button>
          </section>

          <PricingCatalog pricing={pricing} apis={apis} priceDrafts={priceDrafts} setPriceDrafts={setPriceDrafts} onTogglePublished={togglePricingPublished} onEdit={(item) => { setEditingPriceId(item.id); setPriceForm({ apiId: item.apiId, modelId: item.modelId, displayName: item.displayName, category: item.category, billingUnit: item.billingUnit, priceYuan: String(item.unitPriceCents / 100), minDurationSec: String(item.minDurationSec || ''), maxDurationSec: String(item.maxDurationSec || ''), allowedDurations: (item.allowedDurationsSec || []).join(','), allowedResolutions: (item.allowedResolutions || []).join(','), allowedQualities: (item.allowedQualities || []).join(','), defaultQuality: item.defaultQuality || '', qualityRequired: Boolean(item.qualityRequired), maxReferenceImages: String(Number.isInteger(item.maxReferenceImages) ? item.maxReferenceImages : 4), maxReferenceAudios: String(Number.isInteger(item.maxReferenceAudios) ? item.maxReferenceAudios : 0), maxReferenceVideos: String(Number.isInteger(item.maxReferenceVideos) ? item.maxReferenceVideos : 0) }); void loadModelsForPricing(item.apiId, false); }} onDelete={(item) => act(() => apiRequest(`/api/admin/pricing/${item.id}`, { method: 'DELETE' }), '定价已删除')} onSaveBatch={savePricingBatch} />
          ...
          <UserIdentityControls users={users} currentUserId={user?.id} currentUsername={user?.username} onChangeIdentity={changeUserIdentity} />
          <AdminUsers users={users} currentUserId={user?.id} currentUsername={user?.username} recharges={recharges} pricing={pricing} accessUserId={accessUserId} accessDraft={accessDraft} setAccessDraft={setAccessDraft} balanceAdjustments={balanceAdjustments} setBalanceAdjustments={setBalanceAdjustments} currentPassword={adminPassword} act={act} loadModelAccess={loadModelAccess} saveModelAccess={saveModelAccess} />
        </>}
      </div>
    </section>
  </div>;
...
function PricingCatalog({ pricing, apis, priceDrafts, setPriceDrafts, onTogglePublished, onEdit, onDelete, onSaveBatch }: { pricing: Pricing[]; apis: SystemApi[]; priceDrafts: Record<string, string>; setPriceDrafts: (value: Record<string, string> | ((current: Record<string, string>) => Record<string, string>)) => void; onTogglePublished: (item: Pricing) => Promise<unknown>; onEdit: (item: Pricing) => void; onDelete: (item: Pricing) => Promise<unknown>; onSaveBatch: () => Promise<void> }) {
  ...
  return <section className="border border-dark-600 rounded bg-dark-900/20 p-4"><div className="mb-3"><h3 className="text-sm font-medium text-white">模型目录</h3><p className="mt-1 text-xs text-dark-400">按模型类型和系统 API 端口分组，点击端口名称展开或收起模型。</p></div><div className="space-y-3">{categories.map((category) => { const categoryItems = pricing.filter((item) => item.category === category); if (!categoryItems.length) return null; const apiIds = Array.from(new Set(categoryItems.map((item) => item.apiId))); return <div key={category}><h4 className="mb-1 text-xs font-medium text-primary-300">{categoryLabels[category]}</h4><div className="space-y-1">{apiIds.map((apiId) => { const apiItems = categoryItems.filter((item) => item.apiId === apiId); const api = apis.find((item) => item.id === apiId); const key = `${category}:${apiId}`; const isOpen = expanded[key] ?? false; return <div key={key} className="rounded border border-dark-700"><button type="button" className="flex w-full items-center justify-between px-3 py-2 text-left text-xs text-dark-200 hover:bg-dark-800" onClick={() => setExpanded((current) => ({ ...current, [key]: !isOpen }))}><span>{isOpen ? '▾' : '▸'} {api?.name || '未知 API 端口'} <small className="ml-2 text-dark-500">{apiItems.length} 个模型</small></span><span className="text-dark-500">{api?.provider || ''}</span></button>{isOpen && <div className="divide-y divide-dark-700 border-t border-dark-700">{apiItems.map((item) => <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 px-3 py-2 text-sm"><span className="min-w-0 text-dark-200">{item.displayName}<small className="ml-2 text-dark-500">{item.modelId}</small></span><span className="flex items-center gap-2 text-green-400"><input aria-label={`${item.displayName}价格`} className="w-20 border border-dark-600 bg-dark-900 px-2 py-1 text-white" type="number" min="0" step="0.01" value={priceDrafts[item.id] ?? String(item.unitPriceCents / 100)} onChange={(event) => setPriceDrafts((current) => ({ ...current, [item.id]: event.target.value }))} /> / {item.billingUnit === 'second' ? '秒' : item.billingUnit === 'image' ? '张' : '次'}<button type="button" title={item.enabled ? '取消发布' : '发布'} className={item.enabled ? 'text-green-400' : 'text-dark-500'} onClick={() => void onTogglePublished(item)}>{item.enabled ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}</button><button type="button" title="编辑" className="text-primary-400" onClick={() => onEdit(item)}><Pencil className="h-4 w-4" /></button><button type="button" title="删除" className="text-red-400" onClick={() => void onDelete(item)}><Trash2 className="h-4 w-4" /></button></span></div>)}</div>}</div>; })}</div></div>; })}</div><button type="button" onClick={() => void onSaveBatch()} className="mt-3 px-3 py-2 bg-primary-600 text-sm text-white">统一更新价格</button></section>;
}

function UserIdentityControls({ users, currentUserId, currentUsername, onChangeIdentity }: { users: AuthUser[]; currentUserId?: string; currentUsername?: string; onChangeIdentity: (userId: string, accountType: 'user' | 'special' | 'system') => Promise<void> }) {
  const highestUser = String(currentUsername || '').trim().toLowerCase() === 'zhaoyan';
  return <section className="mb-6 rounded border border-dark-600 bg-dark-900/30 p-4"><h3 className="text-sm font-medium text-white">用户身份设置</h3><p className="mt-1 mb-3 text-xs text-dark-400">新注册用户默认为特殊用户且没有模型权限。特殊用户需单独授权；普通用户自动使用全部已发布模型。身份切换会即时同步到下方用户列表，无需刷新页面。</p><div className="grid gap-2 sm:grid-cols-2">{users.map((item) => { const protectedUser = String(item.username || '').trim().toLowerCase() === 'zhaoyan'; return <label key={item.id} className="flex items-center justify-between gap-3 rounded border border-dark-700 bg-dark-900 px-3 py-2 text-xs"><span className="min-w-0"><span className="block truncate text-dark-200">{item.username}</span><span className="block truncate text-dark-500">{item.email}</span></span><select aria-label={`${item.username}用户身份`} className="shrink-0 rounded border border-dark-600 bg-dark-800 px-2 py-1 text-dark-200" disabled={item.id === currentUserId || (item.role === 'system' && (!highestUser || protectedUser))} value={item.role === 'system' ? 'system' : (item.accountType || 'special')} onChange={(event) => { void onChangeIdentity(item.id, event.target.value as 'user' | 'special' | 'system'); }}><option value="special">特殊用户</option><option value="user">普通用户</option><option value="system">系统用户</option></select></label>; })}</div></section>;
}

function AdminUsers({ users, currentUserId, currentUsername, recharges, pricing, accessUserId, accessDraft, setAccessDraft, balanceAdjustments, setBalanceAdjustments, currentPassword, act, loadModelAccess, saveModelAccess }: AdminUsersProps & { currentUsername?: string }) {
  const selectedUser = users.find((item) => item.id === accessUserId);
  const highestUser = String(currentUsername || '').trim().toLowerCase() === 'zhaoyan';
  return <section className="grid lg:grid-cols-2 gap-6"><div><h3 className="text-sm font-medium text-white mb-2">用户、权限与余额</h3><div className="divide-y divide-dark-700">{users.map((item) => <div key={item.id} className={`py-2 grid grid-cols-[1fr_auto] gap-2 items-center text-sm ${item.id === accessUserId ? 'bg-primary-500/10 border-l-2 border-primary-400 pl-2' : ''}`}><span className="text-dark-200">{item.username}{item.id === accessUserId && <span className="ml-2 text-[10px] text-primary-300">当前配置中</span>}<small className="block text-dark-500">{item.email} · {yuan(item.balanceCents)} · {item.accountType === 'special' ? '特殊用户' : item.role === 'system' ? '系统用户' : '普通用户'}</small></span><div className="flex gap-2"><button className={`px-2 py-1 border ${item.id === accessUserId ? 'border-primary-300 bg-primary-500/20 text-white' : 'border-primary-500 text-primary-300'}`} onClick={() => void loadModelAccess(item.id)}>{item.id === accessUserId ? '正在配置' : '模型权限'}</button><div className="flex"><input className="w-24 px-2 py-1 bg-dark-900 border border-dark-600 rounded-l text-white" type="number" step="0.01" value={balanceAdjustments[item.id] || ''} onChange={(event) => setBalanceAdjustments({ ...balanceAdjustments, [item.id]: event.target.value })} placeholder="增减元" /><button title="调整余额" className="px-2 bg-dark-700 border border-dark-600 rounded-r text-green-400" onClick={() => act(() => apiRequest(`/api/admin/users/${item.id}/balance`, { method: 'POST', body: JSON.stringify({ amountCents: Math.round(Number(balanceAdjustments[item.id]) * 100), description: '系统后台调整余额', currentPassword }) }), '用户余额已调整')}><Coins className="w-4 h-4" /></button></div><select className="bg-dark-900 border border-dark-600 rounded px-2 py-1 text-dark-200" disabled={item.id === currentUserId || (item.role === 'system' && (!highestUser || String(item.username || '').trim().toLowerCase() === 'zhaoyan'))} value={item.role} onChange={(event) => act(() => apiRequest(`/api/admin/users/${item.id}/role`, { method: 'PATCH', body: JSON.stringify({ role: event.target.value, currentPassword }) }), '用户角色已更新')}><option value="user">普通用户</option><option value="system">系统用户</option></select></div></div>)}</div>{accessUserId && <div className="mt-4 border border-primary-500/40 p-3 space-y-2"><div><div className="text-sm text-white">特殊用户模型权限</div><div className="text-xs text-primary-300">当前用户：{selectedUser?.username || '未知用户'} · {selectedUser?.email || '未知邮箱'}</div><div className="mt-1 text-[10px] text-dark-400">这里仅授权可见模型和专属价格；用户仍需在自己的“AI 模型配置”中自行选择要使用的模型。</div></div>{pricing.map((item) => { const draft = accessDraft[item.id] || { enabled: false, price: String(item.unitPriceCents / 100) }; return <label key={item.id} className={`flex items-center gap-2 text-xs ${draft.enabled ? 'text-white' : 'text-dark-500'}`}><input type="checkbox" checked={draft.enabled} onChange={(event) => setAccessDraft((current) => ({ ...current, [item.id]: { ...draft, enabled: event.target.checked } }))} /><span className="flex-1">{item.displayName}</span><span className={draft.enabled ? 'text-green-300' : 'text-dark-600'}>{draft.enabled ? '已授权' : '未授权'}</span><input aria-label={`${item.displayName}专属价格`} className="w-20 px-1 py-1 bg-dark-900 border border-dark-600 text-white" type="number" min="0" step="0.01" value={draft.price} onChange={(event) => setAccessDraft((current) => ({ ...current, [item.id]: { ...draft, price: event.target.value } }))} /><span className="text-dark-500">元</span></label>; })}<div className="flex items-center justify-between pt-1"><span className="text-[10px] text-amber-300">授权和价格修改后，请点击保存才会生效</span><button onClick={() => void saveModelAccess()} className="px-3 py-1.5 bg-primary-600 text-white text-xs">保存模型权限</button></div></div>}</div><div><h3 className="text-sm font-medium text-white mb-2">充值审核</h3><div className="divide-y divide-dark-700">{recharges.filter((item) => item.status === 'pending').map((item) => <div key={item.id} className="py-2 flex justify-between items-center text-sm"><span className="text-dark-200">{item.user?.username || item.user?.email}<small className="block text-dark-500">{yuan(item.amountCents)} · {item.note || '无备注'}</small></span><div className="flex gap-1"><button title="通过并入账" className="p-2 text-green-400" onClick={() => act(() => apiRequest(`/api/admin/recharges/${item.id}/review`, { method: 'POST', body: JSON.stringify({ decision: 'approved' }) }), '充值已入账')}><Check className="w-4 h-4" /></button><button title="拒绝" className="p-2 text-red-400" onClick={() => act(() => apiRequest(`/api/admin/recharges/${item.id}/review`, { method: 'POST', body: JSON.stringify({ decision: 'rejected' }) }), '充值申请已拒绝')}><X className="w-4 h-4" /></button></div></div>)}</div></div></section>;
}
